...
CommandStatus APDUProxyImpl::SendRequestAPDU(
	...
	boost::unique_lock<boost::recursive_timed_mutex> lock(this->globalMutex);
...
void APDUProxyImpl::OnPartialResponse(
	...
	{
		boost::unique_lock<boost::recursive_timed_mutex> lock(this->notificationMutex);
...
void APDUProxyImpl::Run(
	...
	while (proxy->active) {
		...
		boost::unique_lock<boost::recursive_timed_mutex> lock(proxy->notificationMutex);
